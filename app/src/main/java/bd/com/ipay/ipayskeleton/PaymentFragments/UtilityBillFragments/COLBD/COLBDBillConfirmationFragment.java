...
public class COLBDBillConfirmationFragment extends IPayAbstractTransactionConfirmationFragment {
	...
	public void onCreate(@Nullable Bundle savedInstanceState) {
		...
		if (getArguments() != null) {
			subscriberId = getArguments().getString(COLBDBillAmountInputFragment.SUBSCRIBER_ID_KEY, "");
	...
	protected void performContinueAction() {
		...
		if (linkThreeBillPayTask == null) {
			...
			linkThreeBillPayRequest = new PaymentRequest(ContactEngine.formatMobileNumberBD("+8801847200199"),
							...
							case Constants.HTTP_RESPONSE_STATUS_OK:
								...
								new Handler().postDelayed(new Runnable() {
									...
									public void run() {
										...
										bundle.putString(COLBDBillAmountInputFragment.SUBSCRIBER_ID_KEY, subscriberId);
                                        bundle.putSerializable(BILL_AMOUNT_KEY, billAmount);
										if (getActivity() instanceof IPayUtilityBillPayActionActivity) {
                                            int maxBackStack=3;
											((IPayUtilityBillPayActionActivity) getActivity()).switchFragment(new COLBDBillSuccessFragment(), bundle, maxBackStack, true);
										}
									}
								}, 2000);
								if (getActivity() != null)
